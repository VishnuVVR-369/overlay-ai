...
> = {
  ...
  disconnected: { colorClass: 'text-glass-text-muted', label: 'IDLE' },
};
  ...
  return (
    <div className="w-full h-full flex items-center justify-center draggable relative">
      <div className="flex items-center gap-3">
        ...
        {isGenerating && (
          <div className="flex items-center gap-1.5 px-2.5 py-1 bg-glass-accent-subtle rounded-full font-mono text-[10px] font-medium tracking-wide">
      ...
      <button
        onClick={onToggle}
        className="absolute top-2 right-2 w-[30px] h-[30px] bg-glass-bg-secondary border border-glass-border-subtle rounded-glass-sm cursor-pointer flex items-center justify-center p-0 transition-all duration-glass-fast non-draggable z-10 hover:bg-glass-bg-hover hover:border-glass-accent hover:scale-105 active:bg-glass-bg-active active:scale-95"
        title="Press Cmd+Shift+M to expand"
        aria-label="Expand overlay"
      >
        <ExpandIcon size={16} strokeWidth={1.5} />
      </button>
    </div>
  );
}
